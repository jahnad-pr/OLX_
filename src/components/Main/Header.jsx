...
export default function Header({ setLogPop }) {
  ...
  const currentLocaction = useLocation()
  ...
  return (
    <>
        ...
        <div className="inner w-full flex gap-[5px] md:gap-5 items-center m-auto flex-wrap py-2 pr-3">
          ...
          {!isSign ? <p onClick={() => { setLogPop(true) }} className='hidden md:block hover:cursor-pointer'>Login</p> :
            <>
              ...
              <img src={isSign.photoURL || 'https://i0.wp.com/mandapublishers.com/wp-content/uploads/2021/05/%E2%80%94Pngtree%E2%80%94user-icon-isolated-on-abstract_5192004.png?w=500&ssl=1'}alt="User"className="w-8 h-8 bg-black rounded-full md:block hidden"/>
      ...
      { currentLocaction.pathname==='/' &&  <CategorySM />}
      <CategoryXL />
      {sell && <Suspense>
